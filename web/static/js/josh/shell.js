...
(function(root, $, _) {
	Josh.Shell = function(config) {
		...
		var self = {
			...
			prompt : function(output, callback) { // BBB Add a promp method to
													// show text from external
				if (callback == null) {
					callback = function() {
					};
				}
			...
			log : function(type, str) {// BBB Shortcut to promp
				// FIXME Escape string
				$("<div />").addClass(type).html(str);
				self.prompt($("<div />").addClass(type).html(str), function() {
				});
			},
			...
			refresh : function() {
				$(id(_input_id)).replaceWith(self.templates.input_cmd({
					id : _input_id
				}));
				self.render();
		...
		function renderOutput(output, callback) {
			...
			$(id(_shell_view_id)).append(self.templates.input_cmd({
				id : _input_id
			}));
			if (_promptHandler) {
				return _promptHandler(function(prompt) {
		...
		function activate() {
			...
			if ($(id(_input_id)).length == 0) {
				_view.append(self.templates.input_cmd({
					id : _input_id
				}));
			}
			self.refresh();
		...
		_readline.onSearchStart(function() {
			$(id(_input_id)).replaceWith(self.templates.input_search({
				id : _input_id
			}));
